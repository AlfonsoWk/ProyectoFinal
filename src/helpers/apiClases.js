export const getClases = async (pagina) => {
  ...
  const response = await fetch(
    ...
    }/clases?token=${token}&role=${role}&pagina=${pagina1}`
  );
...
export const updateCupos = async (id, datos) => {
  console.log("los datos que se reciben son: ", datos);

  const objeto = JSON.stringify(datos);

  const response = await fetch(
    `${import.meta.env.VITE_BACK_URL}/clases/${id}`,
    {
      method: "PUT",
...
export const getClasesUsuarios = async (usuario) => {
  const response = await fetch(
    `${import.meta.env.VITE_BACK_URL}/clases_usuarios?usuario=${usuario}`
  );
  const data = await response.json();
...
export const cancelaReserva = async (id) => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_BACK_URL}/clases_usuarios/${id}`,
      { method: "DELETE" }
    );
    const data = await response.json();
    console.log(data);
